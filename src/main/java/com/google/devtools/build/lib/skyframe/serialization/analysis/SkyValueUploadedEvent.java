...
final class SkyValueUploadedEvent implements BuildEvent {
  ...
  public BuildEventStreamProtos.BuildEvent asStreamProto(BuildEventContext converters) {
    BuildEventStreamProtos.SkyValueUploaded payload =
        BuildEventStreamProtos.SkyValueUploaded.newBuilder()
            .setKey(key.toString())
            .setVersion(version.toString())
            .setFingerprint(fingerprint.toHex())
            .build();

    return GenericBuildEvent.protoChaining(this).setSkyvalueUploaded(payload).build();
  }
}
