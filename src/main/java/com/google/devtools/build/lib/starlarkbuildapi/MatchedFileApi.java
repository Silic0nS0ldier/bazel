 ...
 *
 * <p>Returned by {@code ctx.actions.match_file} (resolves to exactly one regular file) and {@code
 * ctx.actions.match_directory} (resolves to exactly one directory). Because the resolved exec path
 * is unknown until the source trees' contents are known, a {@code MatchedFile} is not a {@link
...
@StarlarkBuiltin(
    ...
    doc =
        "An opaque handle to a single file or directory dynamically resolved from one or more "
            ...
            + "<a href=\"#match_file\"><code>match_file</code></a> and "
            + "<a href=\"#match_directory\"><code>match_directory</code></a>. It is not a "
...
public interface MatchedFileApi extends StarlarkValue {}
