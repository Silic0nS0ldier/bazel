      ...
      case ActionExecutionValue actionExecutionValue -> {
        if (input.isChildOfDeclaredDirectory()) {
          ...
          if (tree != null && !tree.getChildValues().containsKey(input)) {
            // This fires either for a genuinely nondeterministic tree output, or for a declared
            // pick (ctx.actions.pick_file/pick_directory) whose path does not exist in the tree.
            String errorMessage =
                String.format(
                    "child %s does not exist in tree artifact %s (produced by %s); either a"
                        + " ctx.actions.pick_* referenced a path that the tree does not contain, or"
                        + " the tree's generating action is nondeterministic (a previous execution"
                        + " produced the child but a subsequent one did not)",
