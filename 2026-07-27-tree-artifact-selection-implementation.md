...
- New case in `StarlarkCustomCommandLine.VectorArg` alongside directory expansion (`maybeExpandDirectories`, `:372`), flowing into existing formatting (`map_each`, `format_each`, `uniquify`). **Blocker found during implementation:** `expand(...)` (`:899`) receives only an `InputMetadataProvider`, not the consuming action, and once `discoverInputs` prunes the source trees their metadata is *absent* from that provider — so the sketched "resolve through the same engine at expansion time" does not work. Either (a) thread a per-action resolved-selections map into command-line expansion (new plumbing through `CommandLines.expand`), or (b) leave `Args`-backed source trees unpruned so `getTreeMetadata` still succeeds at expansion time (trading the pruning benefit for that selection). Not yet implemented; selections as action *inputs* work independently of this.
...
- **Where resolution results live for `Args`/executable reuse:** discovery installs children as inputs, but the per-selection resolved lists must also reach spawn construction and command-line expansion; likely a small per-action resolved-selections map on the enhanced action instance, populated during discovery.
  ...
  *(Confirmed a blocker during implementation — see Phase 4. Command-line expansion has no action handle, and pruned trees are not in its `InputMetadataProvider`, so neither reuse-from-discovery nor re-resolution works as sketched. Executable resolution is more tractable because it happens inside the action's own `getSpawn`, which can consult a stored resolved map; `Args` needs the map plumbed through `CommandLines.expand`.)*
...
- **Subtree picks vs `pick_directory` representation:** reusing subtree `SpecialArtifact`s assumes `map_directory`'s subtree support is general enough for arbitrary nesting; if it proves template-expansion-specific, `pick_directory` needs its own artifact flavour, which would ripple through `ActionInputMap`'s grandparent walk.
  ...
  *(Validated during implementation — it does NOT work: routing a subtree `SpecialArtifact` as a standalone action input NPEs in `ArtifactFunction.compute` because `ActionExecutionValue.getTreeArtifactValue(subtree)` is `null` — the generating action records a value only for the root tree. `pick_directory` and directory-typed selection members are consequently deferred in v1. A proper fix needs subtree-input routing: `Artifact.key` routing a subtree to the root's generating action, `ArtifactFunction` returning a sub-view, `ActionInputMap`/`SpawnInputExpander` expanding a subtree input, and prefetch/staging handling it.)*
