...
**Exit criteria — met**: a pick behaves as an ordinary file in `DefaultInfo.files`, in runfiles, and in BEP under local execution, and under BwoB `--remote_download_toplevel` only the picked child is fetched (remote-fixture end-to-end). Remaining follow-up: a `sh_test`-style test-data-dependency scenario (the runfiles-manifest path — the load-bearing part — is covered).
...
# Phase 7 — Hardening and graduation *(implemented, with one open item)*

- **Rewinding — verified.** Lost-input recovery for a resolved match child works: `remoteCacheEvictBlobs_whenPrefetchingMatchedChild_incrementalBuildCanContinue` (in `BuildWithoutTheBytesIntegrationTestBase`) evicts the matched child's CAS blob, forces the consumer to re-run, and the incremental build recovers by re-running the tree's generating action — because the child is owned by that action, the existing tree-artifact rewinding/eviction machinery ([#30065](https://github.com/bazelbuild/bazel/issues/30065), [#30103](https://github.com/bazelbuild/bazel/issues/30103)) handles it with no match-specific code.
- **Shared actions / conflict checking — covered by spec value-equality.** `FileMatchSpec` has value `equals`/`hashCode`, and `computeKey` digests it via `addToFingerprint` (never the resolved expansion), so two identical match actions produce equal keys. This is exercised by the aquery proto test, where two actions carrying identical specs dedupe to a single `FileMatch` — direct evidence the specs compare equal by value. No dedicated shared-action integration test was added (constructing genuinely shareable actions is contrived); the value-equality it depends on is what's tested.
- **Skymeld — verified.** `matchInput_underSkymeld_resolvesCorrectly` builds a match-input consumer under `--experimental_merged_skyframe_analysis_execution`; interleaved analysis/execution resolves the match correctly. (The Phase-1 deferred-owner binding is moot — v1 restricts picks to dependency trees, so there is no same-target owner to bind under concurrency.)
- **Path mapping — verified.** `matchInput_underPathStripping_resolvesAndRenders` builds a match input that is also rendered on the command line (scalar `match_file` via `Args.add`) under `--experimental_output_paths=strip`; the resolved child's mapped path both stages and renders correctly. Placeholder fingerprinting composes because the action key digests the spec, not the (mapped or unmapped) resolved path.
- **Memory — verified by construction.** The enhanced action retains only `resolvedMatches` (`ImmutableMap<FileMatchSpec, ImmutableList<Artifact>>`) — resolved artifacts, not any provider. The `MatchResolvingInputMetadataProvider` wrapper is built inline in `getSpawn` and handed to `super.getSpawn`; it is never stored, so no `InputMetadataProvider` is retained past spawn construction.
- **Invalidation granularity — full for picks, footprint-only for matches (the one open item).** Picks deliver *both* footprint and cache-invalidation granularity: `pickConsumer_unmatchedSiblingChurn_doesNotReexecuteConsumer` confirms a sibling change does not re-execute a pick consumer, because a pick's only declared input is the child (no discovery dependency on the whole tree). Matches deliver *footprint* granularity (staging/fetch only the matched children — `toplevelPick_downloadsOnlyPickedChild` and the staging tests) but **not** cache-invalidation: `matchConsumer_unmatchedSiblingChurn_currentlyReexecutesConsumer` guards the current behavior where a sibling change re-executes the consumer. Root cause is the Phase-3 workaround — because scheduling-dependency trees have no metadata during input discovery, a match's source tree is a *declared* input (pruned from staging but still a dependency), so any change to the tree invalidates the consumer even when the resolved child is byte-identical. Closing this needs either metadata for scheduling-dependency trees during discovery, or a discovered-inputs cache path that excludes the source tree's own digest from the consumer's key.
- **Flag flip plan.** Graduate `--experimental_tree_artifact_selection` to on-by-default once (a) the invalidation-granularity item above is resolved or consciously accepted, (b) directory-typed match members as inputs land or stay documented as unsupported, and (c) a rules_js/toolchain-style consumer validates the API against the prototype — mirroring the lazy-downloads PR process. No `--incompatible` flag is needed since the surface is purely additive.
